...
public class MainActivity extends AppCompatActivity {
    ...
    private void selectTextViewToAppend(String number) {
        if (operation_string.equals(operator.NULL.name())) {
            ...
            if (mInputValue1TextView.getText().length() < MAX_CHARACTERS) {
                ...
                MAX_CHARACTERS = 10;
                Toast.makeText(this, "Không thể có hơn 10 số", Toast.LENGTH_LONG).show();
            }
        } else {
            if (mInputValue1TextView.getText().toString().contains(".")) {
                MAX_CHARACTERS++;
    ...
    public void onNumClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.button_dot:
                if (operation_string.equals(operator.NULL.name())) {
                    if (mInputValue1TextView.getText().toString().contains(".")) {
                        Toast.makeText(this, "Không thể có nhiều hơn một dấu thập phân trong một số", Toast.LENGTH_LONG).show();
                    } else {
        ...
        if (!mInputValue1TextView.getText().toString().equals("")) {
            switch (view.getId()) {
                ...
                case R.id.button_log:
                    operation_string = operator.LOG.name();
                    mOperatorTextView.setText("log");
                    break;
                case R.id.button_factorial:
                    operation_string = operator.FAC.name();
                    mOperatorTextView.setText("!");
                    break;
                default:
                    operation_string = operator.NULL.name();
            ...
            operation_string = operator.LOG.name();
            mOperatorTextView.setText("log");
        } else if (view.getId() == R.id.button_factorial) {
            mInputValue1TextView.setText("1");
            operation_string = operator.FAC.name();
            mOperatorTextView.setText("!");
        } else {
            Toast.makeText(this, "Nhập số đầu tiên", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void onEqualsClick(View view) {
        if (mInputValue1TextView.getText().toString().equals("") || mOperatorTextView.getText().toString().equals("") || mInputValue2TextView.getText().toString().equals("")) {
            Toast.makeText(this, "Nhập số và phép tính", Toast.LENGTH_LONG).show();
        } else {
            ...
            switch (operator.valueOf(operation_string)) {
                ...
                case DIV:
                    ...
                    break;
                case MOD:
                    mFinalResultTextView.setText(String.valueOf(mCalculator.mod(number_one, number_two)));
                ...
                case POW:
                    ...
                    break;
                case LOG:
                    mFinalResultTextView.setText(String.valueOf(mCalculator.factorial(number_one)));
                    operation = getString(R.string.button_log) + mInputValue1TextView.getText().toString();
                    break;
                case FAC:
                    mFinalResultTextView.setText(String.valueOf(mCalculator.factorial(number_one)));
                    operation = mInputValue1TextView.getText().toString() + getString(R.string.button_factorial);
                    break;
                case NULL:
    ...
    private enum operator {
        ADD, SUB, MUL, DIV, MOD, ROOT, POW, LOG, FAC, NULL
    }
